...
async def get_buying_list(message):
    global  i
    if i<5:
        await message.answer(f'Название: Product {i}  | Описание: описание {i}  | Цена: {i*100}',reply_markup=kb3)
...
async def product(call):
    if i==2:
        await call.message.answer (prige_list.Product1)
    if i==3:
        await call.message.answer (prige_list.Product2)
    if i==4:
        await call.message.answer (prige_list.Product3)
    if i==5:
        await call.message.answer (prige_list.Product4)


@dp.callback_query_handler (text="product_o")
async def pr_o(call):
    if i==2:
        await call.message.answer (prige_list.Pr_o1)
    if i==3:
        await call.message.answer (prige_list.Pr_o2)
    if i==4:
        await call.message.answer (prige_list.Pr_o3)
    if i==5:
        await call.message.answer (prige_list.Pr_o4)
    await call. answer()
...
async def pr_c(call):
    if i==2:
        await call.message.answer (prige_list.Pr_c1)
    if i==3:
        await call.message.answer (prige_list.Pr_c2)
    if i==4:
        await call.message.answer (prige_list.Pr_c3)
    if i==5:
        await call.message.answer (prige_list.Pr_c4)
